...
void HealthMonitor::setSensors(const QVector<Sensor *> &sensors) { m_sensors = sensors; }
...
void HealthMonitor::setStationIdentity(const QString &id, const QString &name)
{
    ...
    clean.replace('-', '_');
    clean.replace(' ', '_');
    if (!clean.isEmpty()) m_stationId = clean;
    if (!name.trimmed().isEmpty()) m_stationName = name.trimmed();
}
...
void HealthMonitor::stop() { m_timer.stop(); }
void HealthMonitor::evaluateNow() { evaluate(); }
...
void HealthMonitor::evaluate()
{
    ...
    for (Sensor *sensor : m_sensors) {
        ...
        if (!sensor->isAvailable()) {
            level = Offline; reason = "sensor_unavailable";
        } else if (sensor->recoveryPending()) {
            if (sensor->recoverySuccesses() == 0) {
                level = Offline; reason = "recovery_validation_0_of_2";
            } else {
                level = Degraded;
                reason = QString("recovery_validation_%1_of_2").arg(sensor->recoverySuccesses());
            }
        } else if (sensor->consecutiveFailures() >= 2) {
            level = Degraded;
            reason = QString("%1_consecutive_failures").arg(sensor->consecutiveFailures());
        } else if (!sensor->lastValidReading().isValid()) {
            level = Degraded; reason = "awaiting_first_valid_reading";
        } else {
            const int base = qMax(1, sensor->pollIntervalSeconds());
            const qint64 age = sensor->lastValidReading().secsTo(now);
            if (age > qMax(60, base * 3)) {
                level = Offline; reason = QString("stale_%1s").arg(age);
            }
        }
        ...
        if ((level == Healthy || level == Degraded) &&
            ...
            if (rails >= 3) {
                level = Offline;
                reason = QString("sensor_disconnected_boundary_%1").arg(sensor->lastValue(), 0, 'f', 2);
            } else if (rails >= 1) {
                level = Degraded;
                reason = QString("suspect_boundary_%1_%2_of_3").arg(sensor->lastValue(), 0, 'f', 2).arg(rails);
            }
        }
    ...
    if (m_writer) {
        ...
        if (m_writer->consecutiveFailures() >= 3) { cloudLevel = Offline; cloudReason = "api_unreachable"; }
        else if (m_writer->consecutiveFailures() > 0) { cloudLevel = Degraded; cloudReason = "api_retrying"; }
        ...
        const QString queueReason = QString("%1_pending").arg(queued);
        if (queued >= QUEUE_CRITICAL) queueLevel = Critical;
        else if (queued >= QUEUE_WARN) queueLevel = Degraded;
    ...
    if (storage.isValid() && storage.isReady()) {
        ...
        const QString reason = QString("%1_mb_free").arg(available / (1024 * 1024));
        if (available <= DISK_CRITICAL_BYTES) diskLevel = Critical;
        else if (available <= DISK_WARN_BYTES) diskLevel = Degraded;
        updateComponent("storage", diskLevel, reason, now);
    ...
    if (temp >= 0.0) {
        ...
        const QString reason = QString::number(temp, 'f', 1) + "C";
        if (temp >= CPU_CRITICAL_C) cpuLevel = Critical;
        else if (temp >= CPU_WARN_C) cpuLevel = Degraded;
        updateComponent("cpu_temperature", cpuLevel, reason, now);
...
void HealthMonitor::updateComponent(const QString &id, Level level, const QString &reason, const QDateTime &now)
{
    ComponentState &state = m_states[id];
    const bool changed = !state.initialized || state.level != level;
    if (changed) {
        state.level = level; state.changedAt = now; state.initialized = true;
        qInfo().noquote() << QString("Health %1/%2: %3 (%4)").arg(m_stationId, id, levelName(level), reason);
    }
    state.reason = reason;
    publishIfNeeded(id, state, now, changed);
}

void HealthMonitor::publishIfNeeded(const QString &id, ComponentState &state, const QDateTime &now, bool force)
{
    if (!m_writer) return;
    const bool heartbeatComponent = (id == "application" || id == "overall");
    const bool heartbeatDue = heartbeatComponent &&
        (!state.lastPublishedAt.isValid() || state.lastPublishedAt.secsTo(now) >= m_heartbeatSeconds);
    if (!force && !heartbeatDue) return;
...
QString HealthMonitor::snapshotPath() const
{
    ...
    if (root.isEmpty()) root = QDir::homePath() + "/.local/state";
...
void HealthMonitor::writeSnapshot(const QDateTime &now)
{
    ...
    for (auto it = m_states.cbegin(); it != m_states.cend(); ++it) {
        ...
        if (it.value().changedAt.isValid()) c["changed_at"] = it.value().changedAt.toString(Qt::ISODate);
        components[it.key()] = c;
    }
    QJsonObject root;
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qWarning() << "HealthMonitor: cannot write" << snapshotPath(); return;
    }
    file.write(QJsonDocument(root).toJson(QJsonDocument::Indented));
    if (!file.commit()) qWarning() << "HealthMonitor: cannot commit" << snapshotPath();
}
