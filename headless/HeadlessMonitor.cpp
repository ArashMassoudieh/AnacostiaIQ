...
HeadlessMonitor::~HeadlessMonitor() { shutdown(); }

bool HeadlessMonitor::start() {
    if (!config.load(m_configPath)) {
        qCritical().noquote() << "Cannot read" << m_configPath << "—" << config.errorString();
        return false;
    }
    loadConfiguration();
    registerSensors();
    if (sensors.isEmpty()) qWarning() << "No sensors configured — weather polling only";

    int up = 0;
    for (Sensor *s : sensors) {
        if (s->initialize()) {
            ++up;
            qInfo().noquote() << QString("  %1 (%2) ready — every %3 s, %4 sample(s)/reading")
                .arg(s->displayName(), s->id()).arg(effectiveIntervalSeconds(s)).arg(s->samplesPerReading());
        } else {
            qWarning().noquote() << QString("  %1 (%2) unavailable — no GPIO or sensor missing")
                .arg(s->displayName(), s->id());
        }
    }
    qInfo().noquote() << QString("%1 of %2 sensor(s) available").arg(up).arg(sensors.size());
...
void HeadlessMonitor::loadConfiguration() {
    ...
    fetcher.setNoaaGrid(config.noaaOffice(), config.noaaGridX(), config.noaaGridY());
...
void HeadlessMonitor::registerSensors() { sensors = config.createSensors(this); }
...
void HeadlessMonitor::shutdown() {
    ...
    m_stopped = true;
    healthMonitor.stop();
    for (auto it = sensorTimers.cbegin(); it != sensorTimers.cend(); ++it) it.value()->stop();
    if (weatherTimer) weatherTimer->stop();
    for (Sensor *s : sensors) s->cleanup();
    qInfo() << "Monitor stopped; hardware released";
}

int HeadlessMonitor::effectiveIntervalSeconds(Sensor *s) const {
    const int base = s->pollIntervalSeconds() > 0 ? s->pollIntervalSeconds() : pollInterval;
    if (!adaptiveEnabled || !lowFrequency) return base;
...
void HeadlessMonitor::pollSensor(Sensor *s) {
    if (!s || m_stopped) return;
    if (!s->isAvailable()) { healthMonitor.evaluateNow(); return; }
    const double value = s->takeReading();
    if (!Sensor::isValid(value)) {
        qWarning().noquote() << QString("%1: no valid reading").arg(s->displayName());
        healthMonitor.evaluateNow(); return;
    }
    qInfo().noquote() << QString("%1 = %2 %3").arg(s->displayName()).arg(value, 0, 'f', 2).arg(s->unit());
...
void HeadlessMonitor::pollWeather() {
    if (m_stopped) return;
    const QVector<WeatherData> rainAmount = fetcher.getWeatherPrediction(datatype::PrecipitationAmount);
    const QVector<WeatherData> rainProb = fetcher.getWeatherPrediction(datatype::ProbabilityofPrecipitation);
    const QVector<WeatherData> temp = fetcher.getWeatherPrediction(datatype::Temperature);
    if (m_stopped) { qInfo() << "Weather result discarded because shutdown is in progress"; return; }
    qInfo().noquote() << QString("Forecast: %1 precip, %2 probability, %3 temperature point(s)")
    ...
    const RainPolicy::Decision decision = RainPolicy::evaluate(rainProb, QDateTime::currentDateTime(), lookaheadHours, rainThreshold);
    haveForecast = (decision != RainPolicy::Decision::NoDataInWindow);
    if (!haveForecast) qWarning() << "Adaptive polling: no usable precipitation forecast — staying at high frequency";
